...
export default function Filters({ category, initial }: { category: string; initial: ProductSummaryDTO[] }) {
  ...
  return (
    <div className="grid md:grid-cols-[230px_1fr] gap-6 items-start">
      <aside className="bg-white border border-[#F1E7DC] rounded-2xl p-4 shadow-sm md:sticky md:top-20">
        <h4 className="text-xs uppercase font-bold text-[#A99FB4] mb-2">Price (KSh)</h4>
        <div className="flex gap-2 mb-4">
          <input value={min} onChange={(e) => setMin(e.target.value)} placeholder="min" type="number" className="w-full border border-[#E7DACd] rounded-lg p-2 text-xs" />
          <input value={max} onChange={(e) => setMax(e.target.value)} placeholder="max" type="number" className="w-full border border-[#E7DACd] rounded-lg p-2 text-xs" />
        </div>
        ...
        <select value={sort} onChange={(e) => setSort(e.target.value)} className="w-full border border-[#E7DACd] rounded-lg p-2 text-sm">
