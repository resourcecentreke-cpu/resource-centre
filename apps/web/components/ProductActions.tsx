    ...
    <div className="mt-4">
      <div className="flex gap-2 flex-wrap">
        <button onClick={() => setCmp(toggleCompare(slug).includes(slug))}
          className={`px-4 py-2 rounded-full text-sm font-bold border ${cmp ? 'bg-mint text-[#06382a] border-mint' : 'bg-white border-[#D5DAF0]'}`}>
          {cmp ? '✓ In compare' : '+ Compare'}
        </button>
      </div>
      <div className="mt-4 bg-white border border-[#E3E6F4] rounded-2xl p-4">
        <div className="text-sm font-bold mb-2">🔔 Notify me when the price drops below</div>
        <div className="flex gap-2 flex-wrap items-end">
          <input type="number" value={target} onChange={(e) => setTarget(Number(e.target.value))} className="border border-[#D5DAF0] rounded-lg p-2 text-sm w-36" />
          <input type="email" placeholder="email (optional)" value={email} onChange={(e) => setEmail(e.target.value)} className="border border-[#D5DAF0] rounded-lg p-2 text-sm flex-1 min-w-[160px]" />
